...
最初只算「下一集是第几话」，很快就发现会把两件事混在一起：番还在播但我落后了，该催我补；番已经播完了，不该再报「下一集」。

于是抽出一个 `watchState()`，一次给出五个数：
...
列表原本按「更新时间排序取前 8」，结果把三小时**前**更新的番也列了进去，倒计时显示成「3 小时后」——方向反了。现在 `upcomingWithin()` 只收 `next.ms > nowMs` 的条目。

### 4. 截图时上一步的设置漏到了后面每一张

加了壁纸功能之后，截图脚本从 7 张扩到 19 张，于是撞上这个：

- **症状**：只有 `07-wallpaper` 该有壁纸，结果 `09` 到 `19` 全带着壁纸。
- **根因**：整个批次共用一个浏览器 profile，而主题和壁纸是存在 `localStorage` 里的 —— 上一张设的值，下一张自然还看得见。
- **修法**：每张图开拍前，在 `evaluateOnNewDocument` 里**先清空本地存储再播种**。顺手给探针加了反向断言：`不该有壁纸却出现了壁纸（状态串了）` —— 这条正是为了盯住这个坑。

**教训**：截图不是「拍一下」，是「在干净的前提下拍一下」。只要测试之间共享任何持久状态，就会出现这种「单独跑是对的、连起来跑是错的」。

### 5. 两个 `Edit` 同时改一个文件，会互相覆盖

这个坑犯了两次，第二次是写这批测试的时候：

- **症状**：明明看到「Successfully edited」，但新加的测试根本没进文件；跑起来还是 77 个用例。
- **根因**：同一轮里对同一个文件发起两个 `Edit`，两者都基于**同一份原始内容**匹配并写回，后写的那个把先写的覆盖掉了。**两个都报成功，但只有一个真的留下。**
- **修法**：改同一个文件的多个位置，必须**串行**；判断依据也很简单 —— 改完重跑一次，看用例数对不对。

**教训**：报成功不等于改到了。凡是「改完应该能观察到数量变化」的地方，都要实际数一遍，而不是相信返回信息。

### 6. 自动更新：界面和设置都在，但链路是断的

- **症状**：设置面板里「检查更新」按钮、更新源输入框、启动时自动检查，全都齐了；但真点下去只会得到「更新检查不可用」。
- **根因**：这是一条**跨四层的链路**：设置面板 → `core/update.js` 判断 → `platform.checkUpdate` → preload 暴露 → 主进程 IPC。写的时候前两层完成了，后三层还是空的：`preload.cjs` 没暴露 `checkUpdate`，`main.cjs` 没有对应 handler，`core/update.js` 注释里提到的 `electron/updater.cjs` 压根不存在。**每一层单独看都「像完成了」。**
- **修法**：补齐 `electron/updater.cjs`（取数）+ `update:check` IPC + preload 暴露，并顺手把「GitHub 网页地址 → API 地址」这一步从主进程挪到 `core/update.js` 的纯函数 `toApiUrl()` 里 —— 放纯函数层才能进测试，也就不会再出现「这层写了没人调」。
- **顺带补的测试**：`toApiUrl` 的 5 种 GitHub 写法、5 种非 GitHub / 空值输入、以及 `resolveManifestUrl` 的输出确实可直接请求。

**教训**：凡是「跨层」的功能，验收标准不是「每层都写了」，而是**从出口点一次，看能不能走到底**。链路上任何一环空着，前面写得再齐也是白写。

### 7. 假数据全绿，真数据立刻露馅：制作公司取成了人名

接上真实 API 之后第一次跑 `npm run live`，「制作」那一列出来的全是没有意义的字符串 —— 有个人名，还有带说明前缀的credit。

- **根因**：真实 Bangumi infobox 里含「制作」二字的 key 有一大堆 —— `音响制作担当` / `音乐制作` / `制作进行` / `制作进行协力` / `制作管理` / `设定制作` / `美术制作进行` / `CG 制作人` / `OP・ED 动画制作` / `製作`……
  而当时的解析写成了 `/动画制作|製作|制作/` 撞**第一个**匹配，正好撞上排在最前面的 `音响制作担当`，后面挂的是个人名。
- **为什么测试没抓到**：本地 stub 返回的假 infobox 里**只放了一个 `动画制作`**，一个噪声 key 都没有。测试全绿，逻辑照样是错的。
- **修法**：改成按 key **精确分层**匹配（`动画制作` / `动画制作公司` / `制作` 三档），白名单之外一概不认，再滤掉值里的「製作委员会」；同时把真实 infobox 的**噪声 key 组合**（值改成虚构）钉进离线测试，让这个 bug 回不来。
- **顺带修正两条错误断言**：「12 部必须都有评分」「12 部必须都有制作公司」—— 前者会因为「新番还没人打分」误报，后者真实覆盖率本来就只有一半左右。**真实数据的形状不该被当成 bug。**

**教训**：假数据只能验证「解析代码写得对不对」，验证不了「它跟真实数据长得一样吗」。**能连真网络的时候，一定要连一次。** 那些「本地全绿」的测试里躺着的 bug，往往要等接上真东西才第一次露面。

### 8. `window.prompt` 在 Electron 里一调用就抛异常

- **症状**：设置 → 布局 → 预设「改名」，在浏览器里好使，桌面版点了**毫无反应**。
- **根因**：`window.prompt` 在 Electron 渲染进程里**存在但不可用**，一调用就抛 `prompt() is and will not be supported.`。当时的写法是 `globalThis.prompt?.(...)` —— 可选调用链**救不了这种情况**：它只在 `prompt` 本身是 `undefined` 时才短路，而这里它是存在的，只是会抛。
- **修法**：换成行内编辑（输入框 + 确定 / 取消，Enter 提交、Esc 取消），并加了张 `20-preset-rename.png` 和 DOM 探针，专门盯「有没有真的变成输入框」。

**教训**：`?.` 不是万能的防御。浏览器里跑得通不等于桌面壳里跑得通 —— **凡是碰原生弹窗、原生 API 的，两个壳都要各验一次。**
...
那套全虚构的数据没有删，只是搬到了 `test/fixtures/fictional-data.js`，**只用于测试，不进产品**。理由是：虚构数据能验证「解析代码写得对不对」，但验证不了「它跟真实数据长得一样吗」—— 见下面第 7 个坑。

想更新随包数据，重跑 `npm run builtin` 再提交即可；改到的只有 `src/data/builtin/` 一个目录。

### 网络：api.bgm.tv 国内直连不通 —— 这条链路已经真实跑通了

`api.bgm.tv` 在国内一些网络环境下连不上（**实测：域名能解析，但 TCP 直接超时**）。开了代理之后是通的，而这条链路已经拿真实数据完整跑过一遍。

**`npm run live`** 一次做完下面这些：

1. 自动探测本地代理端口（Clash / mihomo / v2rayN 这类常见的十几个都试一遍；也可以用 `ANIME_DESK_PROXY` 直接指定）；
...
| --- | --- |
| 代理探测 | `127.0.0.1:7892` |
| 端点连通 | `https://api.bgm.tv` · 1.5 s |
| 补全 12 部 | 12 成功 / 0 失败 · 用时 6.2 s |
| 话数 / 封面 | 12 / 12 |
| 评分 | 11 / 12 —— 另一部刚开播、还没人打分，留空是对的 |
| 制作公司 | 6 / 12 —— 真实条目常常没这一项，不是解析问题 |
| 封面直链 | `200 · image/jpeg · 407 KB` |

后两行的「不是 12/12」值得单说：**它们不是缺陷，是真实数据的形状。** 断言如果写成「必须全都有」，就会把「新番还没人打分」当成 bug —— 一开始就是这么错的。
...
- 换主题没有过渡动画（有意的：动画会让「变量有没有真的换掉」这件事在截图里变糊）；
